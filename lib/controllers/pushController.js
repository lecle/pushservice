...
exports.create = function(req, res, container) {
    ...
    var doPush = function(query, data, res, next) {
...
function push(data, docs) {
    ...
    if(iosTargets.length > 0)
        iosPush.send(iosTargets, data);

    if(androidTargets.length > 0)
        androidPush.send(androidTargets, data);
}
